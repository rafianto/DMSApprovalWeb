...
@section('title', 'Overview Sent To Do Proccess')
@section('header-title-content', 'Overview Sent To Do Proccess')
          ...
          <div class="card card-primary">
            <div class="card-header">
              <div class="card-title">
                Overview Sent Process DMS No {{ $idd }}
              </div>
            </div>
              <div class="card-body">
                      ...
                      <div class="tab-content">
                        ...
                        <div class="tab-pane" id="subdetaildms">
                          ...
                          <div class="callout callout-info">
                            ...
                            DMS yang sudah Sent. Silahkan di Proses jika setuju Click Approved dan jika tidak Click Cancel.
                          </div>
                ...
                <!-- /.col -->
                <div class="mb-2">
                    <a class="btn btn-success text-white" onclick="proccessApproveOrCancel('approved')"
                        data-shuffle id="btnApprove"
                    > Approved </a>
                    <a class="btn btn-danger text-white" onclick="proccessApproveOrCancel('cancel')"
...
<!-- /.content -->
@include("templates.note_text_approval")
...
<script>
    ...
    function sendApproval() {
        let note = $("#noteText").val() ? $("#noteText").val() : null;

        // hidden modal
        $("#modalFormNoteApproval").modal("hide");
        ...
        if(stateApp == 'cancel')
        {
            $("#btnCancel").html('<i class="fas fa-spinner fa-spin text-white"></i>');
        } else if(stateApp == 'approved'){
        ...
        values.action = stateApp;
        values.note = note;

        $.ajax({
            url: `{{ url('overview/approve-cancel') }}`,
            method: 'POST',
            data: values,
            success: (result) => {
            ...
            error: (xhr, ajaxOptions, thrownError) =>{
                ...
                if(xhr.status == 500)
                {
                    Swal.fire({
                        ...
                        text: `${xhr.responseJSON.message.substr(0, 200)}`,
                        // footer: '<a href="">Why do I have this issue?</a>'
                    });
    ...
    function proccessApproveOrCancel(state)
    {
        stateApp = state
        $("#noteText").val('');
        $("#modalFormNoteApproval").modal("show");
    }
